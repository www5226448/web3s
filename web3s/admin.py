...
class Admin(Module):
    ...
    async def datadir(self):
        return await self.web3.manager.request_blocking("admin_datadir", [])

    @property
    async def nodeInfo(self):
        return await self.web3.manager.request_blocking("admin_nodeInfo", [])

    @property
    async def peers(self):
        return await self.web3.manager.request_blocking("admin_peers", [])
    ...
    async def stopRPC(self):
        return await self.web3.manager.request_blocking("admin_stopRPC", [])

    async def stopWS(self):
        return await self.web3.manager.request_blocking("admin_stopWS", [])
