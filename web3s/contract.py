...
from web3s.utils.contracts import (
    encode_abi,
...
from web3s.utils.datatypes import (
...
from web3s.utils.events import (
    get_event_data,
)
from web3s.utils.filters import (
    construct_event_filter_params,
)
from web3s.utils.function_identifiers import (
    FallbackFn,
)
...
class Contract:
    ...
    :meth:`web3s.eth.Eth.contract` that takes compiled Solidity contract
    ...
    * Deploy a new smart contract using :py:meth:`Contract.deploy`
    ...
    def __init__(self, address=None):
        ...
        """
        if self.web3s is None:
            raise AttributeError(
                'The `Contract` class has not been initialized.  Please use the '
                '`web3s.contract` interface to create your contract class.'
            )
        ...
        self.functions = ContractFunctions(self.abi, self.web3s, self.address)
        self.events = ContractEvents(self.abi, self.web3s, self.address)
        self.fallback = Contract.get_fallback_function(self.abi, self.web3s, self.address)

    @classmethod
    def factory(cls, web3s, class_name=None, **kwargs):
        ...
        setattr(contract, 'functions', ContractFunctions(contract.abi, contract.web3s))
        setattr(contract, 'events', ContractEvents(contract.abi, contract.web3s))
        setattr(contract, 'fallback', Contract.get_fallback_function(contract.abi, contract.web3s))
    ...
    async def deploy(cls, transaction=None, args=None, kwargs=None):
        """
        ...
        txn_hash = await cls.web3s.eth.sendTransaction(deploy_transaction)
        return txn_hash
    ...
    def constructor(cls, *args, **kwargs):
        ...
        return ContractConstructor(cls.web3s,
                                   cls.abi,
                                   cls.bytecode,
                                   *args,
                                   **kwargs)
    ...
    def encodeABI(cls, fn_name, args=None, kwargs=None, data=None):
        ...
        return encode_abi(cls.web3s, fn_abi, fn_arguments, data)

    @combomethod
    @deprecated_for("contract.events.<event name>.createFilter")
    async def eventFilter(self, event_name, filter_params={}):
        """
        ...
        log_filter = await self.web3s.eth.filter(event_filter_params)
    ...
    def estimateGas(self, transaction=None):
        ...
        if self.web3s.eth.defaultAccount is not empty:
            estimate_transaction.setdefault('from', self.web3s.eth.defaultAccount)
    ...
    def call(self, transaction=None):
        ...
        :param transaction: Dictionary of transaction info for web3s interface
        ...
        if self.web3s.eth.defaultAccount is not empty:
            call_transaction.setdefault('from', self.web3s.eth.defaultAccount)
    ...
    def transact(self, transaction=None):
        ...
        .. code-block:: python
            ...
            # Deposit to the `web3s.eth.coinbase` account.
            >>> wallet.functions.deposit().transact({'value': 12345})
            '0x5c504ed432cb51138bcf09aa5e8a410dd4a1e204ef84bfed1be16dfba1b22060'
            # Deposit to some other account using funds from `web3s.eth.coinbase`.
            >>> wallet.functions.deposit(web3s.eth.accounts[1]).transact({'value': 54321})
        ...
        if self.web3s.eth.defaultAccount is not empty:
            transact_transaction.setdefault('from', self.web3s.eth.defaultAccount)
    ...
    def all_functions(self):
        return find_functions_by_identifier(
            self.abi, self.web3s, self.address, lambda _: True
        )
    ...
    def get_function_by_signature(self, signature):
        ...
        fns = find_functions_by_identifier(self.abi, self.web3s, self.address, callable_check)
    ...
    def find_functions_by_name(self, fn_name):
        ...
        return find_functions_by_identifier(
            self.abi, self.web3s, self.address, callable_check
        )
    ...
    @staticmethod
    def get_fallback_function(abi, web3s, address=None):
    ...
    def _encode_constructor_data(cls, args=None, kwargs=None):
        ...
        if constructor_abi:
            ...
            deploy_data = add_0x_prefix(
                encode_abi(cls.web3s, constructor_abi, arguments, data=cls.bytecode)
            )
        else:
...
class ContractConstructor:
    ...
    """
    def __init__(self, web3s, abi, bytecode, *args, **kwargs):
        self.web3s = web3s
        self.abi = abi
    ...
    def _encode_data_in_transaction(self, *args, **kwargs):
        ...
        if constructor_abi:
            ...
            arguments = merge_args_and_kwargs(constructor_abi, args, kwargs)
            data = add_0x_prefix(
                encode_abi(self.web3s, constructor_abi, arguments, data=self.bytecode)
            )
        else:
    ...
    def estimateGas(self, transaction=None):
        ...
        if self.web3s.eth.defaultAccount is not empty:
            estimate_gas_transaction.setdefault('from', self.web3s.eth.defaultAccount)

        estimate_gas_transaction['data'] = self.data_in_transaction

        return self.web3s.eth.estimateGas(estimate_gas_transaction)
    ...
    def transact(self, transaction=None):
        ...
        return self.web3s.eth.sendTransaction(transact_transaction)
    ...
    def buildTransaction(self, transaction=None):
        ...
        if self.web3s.eth.defaultAccount is not empty:
            built_transaction.setdefault('from', self.web3s.eth.defaultAccount)

        built_transaction['data'] = self.data_in_transaction
        built_transaction['to'] = b''
        return fill_transaction_defaults(self.web3s, built_transaction)
...
class ContractFunction:
    ...
    def call(self, transaction=None, block_identifier='latest'):
        ...
        if self.web3s.eth.defaultAccount is not empty:
            print(self.web3s.eth.defaultAccount)
            call_transaction.setdefault('from', self.web3s.eth.defaultAccount)
        ...
        block_id = parse_block_identifier(self.web3s, block_identifier)
    ...
    def _encode_transaction_data(cls):
        return add_0x_prefix(encode_abi(cls.web3s, cls.abi, cls.arguments, cls.selector))
...
async def call_contract_function(
    ...
    if block_id is None:
        return_data = await web3s.eth.call(call_transaction)
    else:
        return_data = await web3s.eth.call(call_transaction, block_identifier=block_id)
    ...
    try:
        ...
        is_missing_code_error = (
            ...
            web3s.eth.getCode(address) in ACCEPTABLE_EMPTY_STRINGS
        )
...
def parse_block_identifier(web3s, block_identifier):
    if isinstance(block_identifier, int):
        return parse_block_identifier_int(web3s, block_identifier)
        ...
        return web3s.eth.getBlock(block_identifier)['number']
    else:
        raise BlockNumberOutofRange


def parse_block_identifier_int(web3s, block_identifier_int):
    if block_identifier_int >= 0:
        block_num = block_identifier_int
    else:
        last_block = web3s.eth.getBlock('latest')['number']
...
def transact_with_contract_function(
    ...
    txn_hash = web3s.eth.sendTransaction(transact_transaction)
    return txn_hash
...
async def estimate_gas_for_function(
    ...
    gas_estimate = await web3s.eth.estimateGas(estimate_transaction)
    return gas_estimate
...
def build_transaction_for_function(
    ...
    prepared_transaction = fill_transaction_defaults(web3s, prepared_transaction)

    return prepared_transaction


def find_functions_by_identifier(contract_abi, web3s, address, callable_check):
