...
from web3s.ens import abis
from web3s.ens.constants import (
    EMPTY_ADDR_HEX,
    REVERSE_REGISTRAR_DOMAIN,
)
from web3s.ens.exceptions import (
    AddressMismatch,
    UnauthorizedError,
    UnownedName,
)
from web3s.ens.utils import (
    address_in,
...
class ENS:
    ...
    def __init__(self, providers=default, addr=None):
        ...
        '''
        self.web3s = init_web3(providers)

        ens_addr = addr if addr else ENS_MAINNET_ADDR
        self.ens = self.web3s.eth.contract(abi=abis.ENS, address=ens_addr)
        self._resolverContract = self.web3s.eth.contract(abi=abis.RESOLVER)

    @classmethod
    def fromWeb3(cls, web3s, addr=None):
        '''
        ...
        :param hex-string addr: the address of the ENS registry on-chain. If not provided,
            ENS.py will default to the mainnet ENS registry address.
        '''
        return cls(web3s.manager.providers, addr=addr)
    ...
    def _assert_control(self, account, name, parent_owned=None):
        if not address_in(account, self.web3s.eth.accounts):
    ...
    def _reverse_registrar(self):
        ...
        return self.web3s.eth.contract(address=addr, abi=abis.REVERSE_REGISTRAR)
