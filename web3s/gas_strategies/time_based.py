...
async def _get_avg_block_time(w3, sample_size):
    latest = await w3.eth.getBlock('latest')
    ...
    oldest = await w3.eth.getBlock(latest['number'] - constrained_sample_size)
    return (latest['timestamp'] - oldest['timestamp']) / constrained_sample_size


async def _get_raw_miner_data(w3, sample_size):
    latest = await w3.eth.getBlock('latest', full_transactions=True)
    ...
    for _ in range(sample_size - 1):
        ...
        block = await w3.eth.getBlock(block['parentHash'], full_transactions=True)
