...
def async_apply_to_return_value(
    callback: Callable[..., T]
) -> Callable[..., Callable[..., T]]:
    def outer(fn: Callable[..., T]) -> Callable[..., T]:
        #the decoration is used for the async request method
        # We would need to type annotate *args and **kwargs but doing so segfaults
        # the PyPy builds. We ignore instead.
        @functools.wraps(fn)
        async def inner(*args, **kwargs) -> T:  # type: ignore
            return callback(await fn(*args, **kwargs))
