...
from web3s.utils.threads import (
    Timeout,
)
...
def fill_transaction_defaults(web3, transaction):
    '''
    if web3s is None, fill as much as possible while offline
    '''
    defaults = {}
...
async def get_block_gas_limit(web3, block_identifier=None):
    if block_identifier is None:
        block_identifier = await web3.eth.blockNumber
    block = await web3.eth.getBlock(block_identifier)
    return block['gasLimit']


async def get_buffered_gas_estimate(web3, transaction, gas_buffer=100000):
    gas_estimate_transaction = dict(**transaction)

    gas_estimate = await web3.eth.estimateGas(gas_estimate_transaction)
...
async def get_required_transaction(web3, transaction_hash):
    current_transaction = await web3.eth.getTransaction(transaction_hash)
